...
def ai_assistant():
    ...
    try:
        ...
        # 使用始皇GPT-4o-mini模型（已配置supports_temperature: False）
        client = create_llm_client("shihuang-gpt4o-mini")
...
<table border="1">
  ...
  <tbody>
    ...
    elif command == 'commitment_general':
        # 生成资格承诺书
        user_prompt = f"""请根据招标文件中"格式自拟"的承诺书或声明要求，生成规范的承诺函文档。

## 分析要求
1. 识别承诺书类型（资格承诺、无违法记录声明、保密承诺等）
2. 提取必须包含的具体条款内容
3. 识别时间范围要求（如"三年内"、"2022年10月至今"等）
4. 提取采购人名称（如有）

## 生成格式规范
**文档结构：**
- 标题：居中，如"投标承诺函"、"无重大违法记录声明"等
- 采购人称呼：顶格，格式为"[采购人名称]："
- 正文内容：
  - 开头语：使用"我司承诺"、"我司声明"或"特此声明"
  - 条款内容：每条独立成段，措辞正式规范
  - 若为资格承诺，在条款前加"我司"使语句完整
  - 若为声明，直接陈述事实
- 落款：
  - 右对齐
  - "投标人/应答人：_____________（加盖公章）"
  - "日期：____年____月____日"

## 内容要点
**重要原则：尽量保留原文表述，仅做必要的语法补充**

处理条款时：
1. **优先使用原文**：直接引用招标文件中的原始表述
2. **最小化改动**：仅在语法不完整时添加必要词语（如主语"我司"、"本次投标"等）
3. **时间表述**：完全保留原文中的时间范围描述（如"三年内（2022年10月至今）"）
4. **专业术语**：保持原文用词，不做同义替换
...
1. 自动判断承诺书类型并使用恰当标题
2. 最大程度保留招标文件原文表述
3. 仅添加必要的主语、状语使句子完整通顺
4. 条款顺序与原文保持一致
5. 预留采购人名称、投标人名称、日期等待填写位置
6. 如原文有特定表述，必须原样使用，不得改写
7. 输出格式为 HTML，使用 <h2> 作为标题，<p> 作为段落，右对齐使用 style="text-align: right;"

请直接输出完整的承诺书文档内容，无需额外解释。"""

    elif command == 'commitment_no_violation':
        # 生成无违法记录声明
        user_prompt = f"""请根据招标文件要求，生成"无重大违法记录声明"。

## 生成格式规范
**文档结构：**
- 标题：<h2 style="text-align: center;">无重大违法记录声明</h2>
- 采购人称呼：[采购人名称]：（如果原文提供）
- 正文内容：
  - 使用"我司在近[X]年内（[起止时间]），在经营活动中没有重大违法记录，特此声明。"
  - 尽量保留原文中的时间范围表述
- 落款（右对齐）：
  - 应答人/投标人：_____________
  - 日期：____年____月____日
...
## 输出要求
1. 完全保留原文中的时间范围（如"三年内（2022年10月至今）"）
2. 使用HTML格式输出
3. 预留采购人名称、投标人名称、日期等待填写
4. 落款使用 style="text-align: right;"

请直接输出完整的声明文档内容，无需额外解释。"""

    elif command == 'commitment_confidential':
        # 生成保密承诺书
        user_prompt = f"""请根据招标文件要求，生成"保密承诺书"。

## 生成格式规范
**文档结构：**
- 标题：<h2 style="text-align: center;">保密承诺书</h2>
- 采购人称呼：[采购人名称]：（如果原文提供）
- 正文内容：
  - 开头语：使用"我司承诺"
  - 保密条款：根据原文要求生成，主要包括：
...
2. 仅添加必要的主语"我司"使句子完整
3. 使用HTML格式输出
4. 预留采购人名称、投标人名称、日期等待填写
5. 落款使用 style="text-align: right;"
