-- 为parser_debug_tests表添加Gemini解析器支持
-- 执行时间：2025-01-XX

-- 1. 添加Gemini解析结果列
ALTER TABLE parser_debug_tests ADD COLUMN gemini_result TEXT;

-- 2. 添加Gemini性能指标
ALTER TABLE parser_debug_tests ADD COLUMN gemini_elapsed REAL;

-- 3. 添加Gemini章节计数
ALTER TABLE parser_debug_tests ADD COLUMN gemini_chapters_count INTEGER DEFAULT 0;

-- 4. 添加Gemini准确率指标
ALTER TABLE parser_debug_tests ADD COLUMN gemini_precision REAL;
ALTER TABLE parser_debug_tests ADD COLUMN gemini_recall REAL;
ALTER TABLE parser_debug_tests ADD COLUMN gemini_f1 REAL;

-- 5. 重新创建summary视图以包含Gemini列
