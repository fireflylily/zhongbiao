...
要求: 专业、清晰、中文"""
    ...
    def _build_basic_prompt(self, chapter: Dict[str, Any], analysis: Dict[str, Any]) -> str:
        ...
        # 🔧 修复：保留所有要点，不截断（之前只取前3个导致表格内容丢失）
        # 如果要点过多，分组显示，但不丢弃任何信息
        if content_hints:
...
2. **专业、清晰、突出技术优势**
3. 使用中文
4. 输出纯文本段落，适合直接放入Word文档
5. 不要使用任何Markdown标记（如 ##、**、*、-、1.等）
6. 如需列举要点，使用带圈数字"① ② ③ ④ ⑤"或"第一点、第二点"等表述，避免使用(1)(2)(3)
7. 如需强调，使用括号或引号，不使用**加粗**"""
...
2. **业务针对性**: 紧扣招标项目的具体场景和需求，避免通用技术描述
3. **能力证明**: 优先使用量化数据（用户数、数据量、集群规模、覆盖范围等）展示实力
4. **具体可落地**: 说明具体的交付内容、技术参数、服务方式、更新频率
5. **语言风格**:
...
7. **格式规范**:
   ...
   - 强调时使用【】或""，不使用**加粗**"""
    ...
    def _extract_project_context(self, analysis: Dict[str, Any]) -> str:
        ...
        # 🔧 修复：提取关键技术要求 - 不再截断，保留更多上下文
        key_requirements = []
        for category in categories[:3]:  # 增加到前3个类别
            points = category.get('key_points', [])
            key_requirements.extend(points[:3])  # 每个类别取前3个要点（从2增加到3）
        if key_requirements:
            req_text = '、'.join(key_requirements[:6])  # 最多6个要求（从3增加到6）
