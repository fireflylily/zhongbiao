...
**[解析文档结构完整工作流程](./ParseDocumentStructure.md)**

详细说明"解析文档结构"按钮的完整工作流程,包括:

- 前端流程 (按钮触发 → API调用 → 结果渲染)
- 后端API处理 (参数验证 → 文件处理 → 任务创建)
- 文档结构解析器核心逻辑 (三种解析模式)
  - Mode 1: TOC提取 (目录存在时)
  - Mode 2: 语义锚点匹配 (模糊匹配目录项与正文)
  - Mode 3: 样式识别 (无目录时,基于Heading样式/字体/编号模式)
- 87种章节编号模式详解
- 智能推荐系统 (白名单/黑名单关键词)
- 案例分析: "数字人民币"项目为何使用样式识别模式
- 常见问题: 为什么解析结果与实际目录不一致
- 数据库存储结构
- 改进建议 (短期/中期/长期)
...
- **2025-11-04**:添加"技术细节文档"章节,链接到ParseDocumentStructure.md (解析文档结构完整工作流程)
