                        ...
                        if match:
                            ...
                            if field_info.get('prevent_duplicate_label', False) and field_name == '电子邮件':
                                ...
                                if '电话：' in search_text and '电子邮箱：' in search_text:
                                    # 如果已经有"电话：xxx电子邮箱："的完整结构，避免重复处理
                                    if re.search(r'电话：[^电子邮箱]*电子邮箱：', search_text):
                                        logger.info(f"段落 #{para_idx} 复合段落已包含电话+电子邮箱结构，跳过处理")
                                        continue
        ...
        try:
            ...
            if is_compound_paragraph and preserve_trailing:
                ...
                if success:
                    ...
                    logger.warning("复合段落跨run替换失败，尝试整体替换策略")
                    # 降级到整体替换策略
