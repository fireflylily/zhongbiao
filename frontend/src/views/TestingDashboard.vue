      ...
      <div class="control-panel">
        <el-card shadow="never">
          ...
          <div class="run-options">
            <!-- 测试环境选择：后端/前端 -->
            <div class="test-env-selector">
              <span class="selector-label">测试环境:</span>
              <el-radio-group v-model="testEnv" :disabled="isRunning" size="default">
                <el-radio-button value="backend">🐍 后端测试</el-radio-button>
                <el-radio-button value="frontend">🎨 前端测试</el-radio-button>
              </el-radio-group>
            </div>

            <!-- 后端测试选项 -->
            <div v-if="testEnv === 'backend'" class="test-options">
            ...
            <!-- 前端测试选项 -->
            <div v-if="testEnv === 'frontend'" class="test-options">
              <el-select
                v-model="frontendModule"
                placeholder="选择模块（留空=全部）"
                clearable
                :disabled="isRunning"
                style="width: 300px;"
              >
                <el-option label="全部前端测试" value="" />
                <el-option label="商务应答模块" value="Business/Response" />
                <el-option label="技术方案模块" value="Business/TechProposal" />
                <el-option label="工具函数" value="utils" />
            ...
            <el-button
              ...
              size="large"
            >
              {{ isRunning ? '测试运行中...' : (testEnv === 'frontend' ? '运行前端测试' : '运行后端测试') }}
            </el-button>
          </div>
        </el-card>
      </div>
          ...
          <template #header>
            <div class="coverage-header">
              ...
              <div class="coverage-actions">
                <el-button size="small" @click="openNativeReport('coverage')" type="primary" plain>
                  📊 查看完整HTML报告
                </el-button>
                <el-progress
          ...
          <template #header>
            <div class="logs-header">
              ...
              <div class="header-actions">
                ...
                <el-button size="small" @click="openNativeReport('test')" type="success" plain>
                  📄 查看HTML测试报告
                </el-button>
...
const frontendModule = ref<string>('')  // 新增：前端模块选择
...
const startTest = async () => {
  ...
  try {
    ...
    if (testEnv.value === 'frontend') {
      // 运行前端测试
      ElMessage.info('正在运行前端Vitest测试...')

      response = await fetch('/api/testing/run', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          test_type: 'frontend',
          test_module: frontendModule.value
        })
      })
      ...
      if (data.success) {
        ElMessage.success(`前端测试完成！通过: ${data.passed}, 失败: ${data.failed}`)
        ...
        currentTask.value = {
          ...
          logs: data.output ? data.output.split('\n') : [],
          test_type: 'frontend',
        ...
        ElMessage.error('前端测试运行失败: ' + (data.error || '未知错误'))
      }
    } else {
      // 运行后端测试（原有逻辑）
      response = await testRunnerApi.runTests(testType.value, enableCoverage.value, true)
 ...
 * 打开原生HTML报告（新窗口）
 */
const openNativeReport = (type: 'test' | 'coverage') => {
  const url = type === 'test'
    ? '/api/testing/view/report'
    : '/api/testing/view/coverage'
