...
# 始皇API通用调用函数
def call_shihuang_api(prompt, max_tokens=1500):
    """
    调用始皇API的通用函数
    """
    url = f"{SHIHUANG_BASE_URL}/chat/completions"
    
    headers = {
        'Content-Type': 'application/json',
        'Authorization': f'Bearer {SHIHUANG_API_KEY}'
    }
    ...
    try:
        ...
        if 'choices' in data and len(data['choices']) > 0:
            ...
            print(f"始皇API错误: {data['error'].get('message', '未知错误')}")
            return None
        else:
        ...
        print(f"调用始皇API时发生错误: {e}")
        return None

# 函数：调用始皇API生成1000字的解决方案
def generate_solution_from_chatgpt(c_content):
    prompt = f"{Prompt_Content} {c_content}"
    result = call_shihuang_api(prompt, max_tokens=2000)
    
    if result:
        print('----生成内容:', result)
        return result
    else:
        return "生成解决方案时出现错误，请检查API配置。"

# 函数：调用始皇API重写Word中的文本内容
def rewrite_word_content(c_column_content, word_content):
    ...
    result = call_shihuang_api(prompt, max_tokens=800)
    
    if result:
        print('----重写内容:', result)
        return result
    else:
        print(f"重写内容时出现错误，返回原内容")
        return word_content  # 如果API调用失败，返回原始内容

# 函数：调用始皇API，将文本缩减为15字以内的标题
def shorten_text(text):
    prompt = f"{Prompt_Title}'{text}'"
    result = call_shihuang_api(prompt, max_tokens=50)
    
    if result:
        cleaned_title = result.replace("。", "")  # 去除中文句号"。"
        ...
        # 如果API调用失败，返回原文前10个字符作为备选
        fallback_title = text[:10]
        if key_flag == 1:
            if '★' in text:
...
# 函数：调用始皇API，优化需求说明满足需求条件
def optimize_description(text):
    prompt = f"{Prompt_Answer}'{text}'"
    result = call_shihuang_api(prompt, max_tokens=500)
    
    if result:
        return result
    else:
        return f"完全支持。系统针对'{text[:20]}...'相关需求提供全面的功能支持。"
