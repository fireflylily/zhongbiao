...
1. **Web层** (`ai_tender_system/web/`):
   - 模块化Flask应用 (`app.py` - 429行，已完成重构 ✅)
   - 基于Blueprint的API路由（9个核心蓝图）
   - 静态资源管理，支持动态CSS加载
   - 已启用CSRF保护和性能优化（Gzip压缩、资源缓存）
...
2. 在 `web/app.py` 中注册blueprint（通过 `register_all_blueprints()`）
3. 为POST/PUT/DELETE端点添加CSRF保护
4. 前端：使用 `window.apiClient` 或 `csrf-protection.js` 工具发起请求

**现有Blueprint清单**：
- `auth_bp.py` - 登录认证
- `pages_bp.py` - 页面路由
- `api_core_bp.py` - 核心API（健康检查、配置）
- `api_files_bp.py` - 文件上传下载
- `api_tender_bp.py` - 招标信息提取
- `api_business_bp.py` - 商务应答处理
- `api_tech_bp.py` - 技术需求回复
- `api_companies_bp.py` - 公司管理
- `api_projects_bp.py` - 项目管理
- `api_editor_bp.py` - 文档编辑器
- `api_models_bp.py` - AI模型管理
- `api_tender_processing_bp.py` - 标书智能处理
- `api_tender_management_bp.py` - 标书管理列表

### Web层重构成果（2025年10月完成）✅

**重构前**：`app.py` 单体应用 - 3,248行
...
4. `web/app.py` - Flask应用工厂（已模块化，429行）
5. `web/blueprints/` - 13个功能蓝图（认证、API、业务逻辑）
6. `modules/business_response/qualification_matcher.py` - 核心资质逻辑
7. `modules/tender_info/extractor.py` - 招标需求提取（2,000+行）

**前端关键文件**：
1. `static/js/core/global-state-manager.js` - 状态管理
2. `static/js/core/api-client.js` - 统一API调用
3. `static/js/utils/sse-client.js` - SSE流式处理
4. `static/js/hitl-config-manager.js` - HITL工作流（复杂）
5. `static/js/components/navigation-manager.js` - 标签页导航

## 已知问题与技术债务

1. ~~**单体 `app.py`**~~ ✅ 已完成重构（减少87%代码，拆分为13个Blueprint）
2. **测试覆盖率**：仅11% - 优先改进领域
3. **硬编码值**：部分API端点存在魔术数字
4. **错误处理**：端点间错误响应不一致
5. ~~**CSS加载**~~ ✅ 已修复 - 动态CSS懒加载已实现
6. **文档**：部分模块缺少文档字符串
7. **前端模块拆分**：`tender-processing-step3/` 目录仍有2,761行代码需拆分
