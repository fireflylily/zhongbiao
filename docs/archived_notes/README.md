...
项目的 `.gitignore` 忽略了以下关键运行时数据：
- ❌ 数据库文件（`*.db`） - 企业信息、资质、案例等数据
- ❌ 环境配置（`.env`） - API密钥和配置
- ❌ 上传文件（`data/uploads/`） - 用户上传的文档
- ❌ 输出文件（`data/outputs/`） - 生成的标书

这导致在新机器上 clone 代码后，功能数据全部丢失。
...
# 4. 配置API密钥
vim ai_tender_system/.env  # 填入实际API密钥
...
| 工具 | 功能 | 命令 |
|------|------|------|
| **环境检查** | 检测缺失的配置、数据库、文件 | `python3 scripts/check_env.py` |
| **完整备份** | 创建数据库+文件备份 | `python3 scripts/backup_environment.py` |
| **环境恢复** | 从备份恢复完整环境 | `python3 scripts/restore_environment.py backup_*.tar.gz` |
| **数据库导出** | 导出数据库为SQL | `python3 scripts/export_database.py` |

### 详细文档

完整的环境同步指南请查看：[ENVIRONMENT_SYNC_GUIDE.md](ENVIRONMENT_SYNC_GUIDE.md)
